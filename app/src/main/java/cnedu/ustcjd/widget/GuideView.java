...
public class GuideView extends RelativeLayout implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    public void showOnce() {
        if (targetView != null) {
            mContext.getSharedPreferences(TAG, Context.MODE_PRIVATE).edit().putBoolean(generateUniqueId(targetView), true).commit();
        }
    }
    ...
    private boolean hasShown() {
        if (targetView == null)
            return true;
        return mContext.getSharedPreferences(TAG, Context.MODE_PRIVATE).getBoolean(generateUniqueId(targetView), false);
    }
...
//        ((FrameLayout) ((Activity) mContext).getWindow().getDecorView()).addView(this);
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (backgroundColor != 0)
            ...
            bgPaint.setColor(getResources().getColor(R.color.clr_guide_view_shadow));
